## Practice Python

### Implementation
- [Searching Algorithms](implementations/searching_algorithms/README.md) (Algorithms)
- [Sorting Algorithms](implementations/sorting_algorithms/README.md) (Algorithms)
- [Django Templates and Reusables](./implementations/django/readme.md) (Django)

### Practice Projects
- [Calculator](projects/calculator/README.md) (GUI Application using Tkinter)
- [Markdown Viewer TUI](projects/markdown_viewer_tui/readme.md) (CLI Application using Textual)
- [Snake and Bait](projects/snake_and_bait/README.md) (GUI Game using pygame)
- [Tetris](projects/tetris/readme.md) (GUI Game)
- [Snake and Ladder](projects/snake_ladder/readme.md) (GUI Game using TKinter)
- [2048 game](projects/the_2048_game/readme.md) (GUI Game using Tkinter)
- [Online Tic-Tac-Toe (Centralized Server)](./projects/online_tic_tac_toe_centralized/readme.md) (Networked Game with GUI using Tkinter)
- [Online Tic-Tac-Toe(Peer to Peer)](./projects/online_tic_tac_toe_peer_to_peer/readme.md) (Networked Game with GUI using Tkinter)
- [AI Powered Flashcard](./projects/ai_powered_flashcard/readme.md) (GUI based Trad AI Application)
- [Food Recommendation Expert System](./projects/food_recommendation_expert_system/readme.md) (Trad AI Expert System)
- [Regex to Finite Automata Visualizer](./projects/regex_nfa_dfa_visualizer/readme.md) (ToC concept GUI Visualizer)
- [Turing Machine Simulator](./projects/turing_machine_simulator/readme.md) (ToC Concept Simulator)
- [Logic Expression Evaluator](./projects/logic_expression_evaluator/readme.md) (CD Concept based Application)
- [Lexical Analyzer (Mini Lex)](./projects/mini_lex/readme.md) (CD Concept based Application)
- [Filesystem in a File](./projects/filesystem_in_a_file/readme.md) (File System Simulator inside a file)
- [Quiz Management System](./projects/flask/quiz_management_system/readme.md) (Quiz Management System using Flask)

<!-- 
### Mini Projects
- [Server](projects/server/README.md)
- [Trading Engine](projects/trading_engine/README.md)
 -->